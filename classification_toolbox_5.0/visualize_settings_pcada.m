function varargout = visualize_settings_pcada(varargin)

% visualize_settings_pcada opens a graphical interface for prepearing
% settings of PCA-DA
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @visualize_settings_pcada_OpeningFcn, ...
                   'gui_OutputFcn',  @visualize_settings_pcada_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before visualize_settings_pcada_pc is made visible.
function visualize_settings_pcada_OpeningFcn(hObject, eventdata, handles, varargin)
handles.output = hObject;
set(hObject,'Position',[103.8571 38 57.2857 21.4706]);
set(handles.visualize_settings_pcada,'Position',[103.8571 38 57.2857 21.4706]);
set(handles.text19,'Position',[4.6 11.2308 22.8 1.1538]);
set(handles.pop_menu_disc,'Position',[4.4 9.4615 26.6 1.6923]);
...
movegui(handles.visualize_settings_pcada,'center')
...
set(handles.pop_menu_disc,'String',str_disp);
...
if handles.model_is_present == 2 & handles.disable_comp == 0
    if strcmp(model_loaded.type,'pcalda') | strcmp(model_loaded.type,'pcaqda')
        if strcmp(model_loaded.settings.modelpca.settings.param.pret_type,'none')
            set_this = 1;
        elseif strcmp(model_loaded.settings.modelpca.settings.param.pret_type,'cent')
            set_this = 2;
        else
            set_this = 3;
        end
        set(handles.pop_menu_scaling,'Value',set_this);
        if strcmp(model_loaded.type,'pcaqda')
            set(handles.pop_menu_disc,'Value',2);
        end
        set(handles.pop_menu_numcomp,'Value',size(model_loaded.settings.modelpca.T,2));
    else
...
uiwait(handles.visualize_settings_pcada);

% --- Outputs from this function are returned to the command line.
function varargout = visualize_settings_pcada_OutputFcn(hObject, eventdata, handles)
len = length(handles);
if len > 0
    ...
    if get(handles.pop_menu_disc,'Value')  == 1
        varargout{6} = 'pcalda';
    else
        varargout{6} = 'pcaqda';
    end
    delete(handles.visualize_settings_pcada)
else
...
handles.domodel = 1;
guidata(hObject,handles)
uiresume(handles.visualize_settings_pcada)
...
% --- Executes during object creation, after setting all properties.
function pop_menu_disc_CreateFcn(hObject, eventdata, handles)
if ispc
    set(hObject,'BackgroundColor','white');
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
...
% --- Executes on selection change in pop_menu_disc.
function pop_menu_disc_Callback(hObject, eventdata, handles)
